...
将来的には `Lisp -> Python` など別の変換器を増やして行きたいなと思っています。

※これはゲーム内のウィンドウで別言語を実行するためのツールではありません。主にIDEなどを用いながら別言語のコードを書いていただき、保存するたびゲームで使用できるPython風コードに反映されるという形をとっています。
...
6. 既に自動生成されているrs_src/main.rsなんかを編集・保存すると、ゲームに反映されているようになっているはずです。
