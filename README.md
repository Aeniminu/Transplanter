...
9. 自動生成されている `rs_src/main.rs` を編集して保存します。
...
`rs_src` はあなたが書く場所です。名前は `rs_src` のままですが、言語モードを `Lisp` にすると `.scm` / `.lisp` も使えます。ゲームが読むのは Save フォルダ内の `.py` です。

言語モードはセーブごとに選べます。`Rust` では `.rs` だけ、`Lisp` では `.scm` / `.lisp` だけを変換します。`自動` は上級者向けの混在モードで、`.rs` / `.scm` / `.lisp` を拡張子から判定して変換します。

## 最初の main.rs

初回起動で作られる `rs_src/main.rs` は、次のようなRustコードとして編集できます。
...
言語モードを `Lisp` にすると、`rs_src/main.scm` が自動作成されます。`.scm` または `.lisp` を `rs_src` に置くと、同じように `.py` へ変換できます。
