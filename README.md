...
古いバージョンから `rs_src` を使っている場合も、`transplanter.toml` の `src_dir` が実在する `rs_src` を指していればそのまま使えます。`rs_src` が見つからない場合は、新しい既定の `play_src` に最初のファイルを作ります。古い作業フォルダ直下の `transplanter.toml` は、新しい `.transplanter/transplanter.toml` へ移行されます。
