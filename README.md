...
`rs_src` はあなたが書く場所です。名前は `rs_src` のままですが、`.rs` だけでなく `.scm` / `.lisp` も置けます。ゲームが読むのは Save フォルダ内の `.py` です。
...
`.scm` または `.lisp` を `rs_src` に置くと、同じように `.py` へ変換できます。
...
同じ出力名になるファイルは同時に置けません。たとえば `main.rs` と `main.scm` はどちらも `main.py` になるため、片方だけにしてください。
